...
| 构建方式 | 输出目录 | 用途 |
|---|---|---|
| `cmake --preset dev-mingw` | **`build-dev/bin/`** | 开发期 MinGW 构建（含测试 exe） |
| `cmake --preset windows-x64-debug` | **`build/bin/`** | MSVC x64 构建（需 VS2022 环境） |
| `bash tools/build-release.sh` | **`dist/`** | 绿色发布（MinGW Release，仅 remote_tool.exe + agent.exe） |
